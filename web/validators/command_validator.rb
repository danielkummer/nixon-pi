...
class CommandValidator < ActiveModel::Validator
  ...
  def validate_animation(record)
    ...
    record.errors.add(:options, "options must include a start_value key") if !option_hash.has_key?(:start_value) or option_hash[:start_value].to_s.blank?
    record.errors.add(:options, "options must include a goto_state key") if !option_hash.has_key?(:goto_state) or option_hash[:goto_state].to_s.blank?
    record.errors.add(:options, "options must include a goto_target key") if !option_hash.has_key?(:goto_target) or option_hash[:goto_target].to_s.blank?
  end
end