      ...
      def initialize
        ...
        @incomming_commands_queue.subscribe(consumer_tag: 'command_consumer') do |_delivery_info, metadata, data|
          if metadata[:content_type] == 'application/json'
            ...
            begin
              data_hash =  ActiveSupport::HashWithIndifferentAccess.new(JSON.parse(data))
            rescue Exception => e
              log.error(e.message)
            end
        ...
        @receivers[target].each do |receiver|
          if receiver.respond_to?(:handle_command)
            ...
            if validator_model.valid?
              data.delete_if { |k, v| !receiver.class.available_commands.include?(k) || v.nil? }
