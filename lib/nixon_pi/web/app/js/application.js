...
(function (nixiecode) {
    ...
    var initTubeForms = function () {

        $animationOptionsInput = $('#tube-animation-options');

        //load example JSONs based on selected animation
        $('#tube-animation-select').on('change', function (event) {
            var selectedAnimation = $(this).val();

            if (selectedAnimation == 'single_fly_in') {
                $animationOptionsInput.val('{\n start_value: "000123456789",\n goto_state: "time",\n goto_target: "tubes" \n}');
            } else if (selectedAnimation == 'switch_numbers') {
                $animationOptionsInput.val('{\n start_value: "777777777777",\n turnarounds: "3",\n goto_state: "time",\n goto_target: "tubes" \n}');
            }
        });

        $('.tube-form').submit(function (event) {
            event.preventDefault();
            var data = $(this).serializeJSON();

            //catch special case for meeting ticker where we have to combine the both fields to form the new "value" field
            if(data.hasOwnProperty('participants') && data.hasOwnProperty('rate')) {
                data.value = data.participants + ':' + data.rate;
            }
